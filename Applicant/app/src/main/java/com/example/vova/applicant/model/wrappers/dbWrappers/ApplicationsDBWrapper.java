...
public class ApplicationsDBWrapper extends BaseDBWrapper<ApplicationsInfo> {

    public ApplicationsDBWrapper(Context context) {
        super(context, DBConstants.ApplicationsTable.TABLE_NAME);
    }
...
//    }
    ...
    public ArrayList<ApplicationsInfo> getAllApplicationsById(long nId) {
        ArrayList<ApplicationsInfo> arrResult = new ArrayList<>();
        SQLiteDatabase database = getReadable();
        String strRequest = DBConstants.ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_SPECIALITY_ID + "=?";
...
//        String strRequest = ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_SPECIALITY_ID + "=? AND "
//                + DBConstants.ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_NAME + "=?";
...
//    }
    ...
    public ApplicationsInfo getApplicationById(long nId) {
        ...
        SQLiteDatabase database = getReadable();
        String strRequest = ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_ID + "=?";
        String arrArgs[] = new String[]{Long.toString(nId)};
        Cursor cursor = database.query(getTableName(), null, strRequest, arrArgs, null, null, null );
        try{
            if (cursor != null && cursor.moveToFirst()){
    ...
    public ArrayList<ApplicationsInfo> getAllApplicationsBySearchString(long nId, String strSearch){
        ...
        String strRequest = ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_SPECIALITY_ID + "=? AND "
                + DBConstants.ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_NAME + " LIKE ? OR "
                + DBConstants.ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_TOTAL_SCORE + " LIKE ? OR "
                + ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_MARK_DOCUMENT + " LIKE ? OR "
                + DBConstants.ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_MARK_TEST + " LIKE ? ";
    ...
    public void updateAllItems(ArrayList<ApplicationsInfo> applicationsItems) {
        ...
        String strRequest = DBConstants.ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_SPECIALITY_ID + "=? AND "
                + DBConstants.ApplicationsTable.Cols.APPLICATIONS_INFO_FIELD_NAME + "=?";
